<template>
  <div>
    <PageTitle icon="fas fa-file" main="Livros Devolvidos"
      sub="Relatórios" />
    <div class="report">
      <b-form-group>
  ...
  methods: {
    applyFilters() {
      ...
      if (this.filterDate==true) {
        filters += `&initialReturnedAt=${this.initialDate}&finalReturnedAt=${this.finalDate}`
      } 
    ...
    generate() {
      this.loans = [];
      const url = `${baseApiUrl}/loans?status=finished` + this.applyFilters()
